...
class FDTD:
    ...
    def __init__(self, filename = None, hide = False, serverArgs = {}, remoteArgs = {}, units = NANO):
        ...
        filename: str (optional)
            A single string containing either as script filename or a project filename. 
            When the parameter is a project filename, the project will be loaded. 
            When the parameter is a script filename, it will be evaluated. 
        
        hide: bool (optional, default is false)
            Shows or hides the Lumerical GUI/CAD environment on startup. 
            When set to True, all pop-up messages that normally appear in the GUI will also be disabled.

        serverArgs: dict (optional)
            Pass command line arguments to the product during launch as a dictionary.

            Example: serverArgs = {'use-solve':True, 'platform':'offscreen', 'threads': '2’}
        
        remoteArgs: dict
            Pass connection information as a dictionary. 
            Only used when using the Python API remotely on a Linux machine that is 
            running the Interop Server.

            Example:
                remoteArgs = { "hostname": "192.168.215.129", "port": 8989 }

        units: float (optional, default is nm)
            Units multplier used for setting object dimensions and wavelengths.
        """
        connector = Connector
